...
> **Canonical copy.** This file (`plugins/seo-rescue/references/SISTRIX_MONDAY_RECOVERY_CHECK.md`) is the maintained specification the plugin skills read; the repo-root `SISTRIX_MONDAY_RECOVERY_CHECK.md` is a pointer stub.
...
No exports are committed. The CSVs the user feeds in are protected by the repository's `.gitignore` (repo root, not shipped in the plugin package) via entries for `sistrix-exports/`, `*_sistrix*.csv`, `*_visibility*.csv`, `*_keywords*.csv`, `*_ranking*.csv`.
...
See Appendix B — Per-cluster stage classification (cluster identification, per-cluster output table, global-stage derivation).
...
### 12. Conversion rate validation layer (moved to Appendix A)

See Appendix A — Conversion-rate validation layer (VI-trend × CR-trend interpretation matrix, CR baseline, stock-out interaction).
...
Files to protect per the repository's `.gitignore` (repo root, not shipped in the plugin package):
...
When the user runs the skill in private-local-data or sensitive-client-data mode (see `ARCHITECTURE.md` §5 "Privacy and client data" at the repo root — not shipped in the plugin package — for the privacy posture), the output filename is further hashed and the path is mode-0700.

## What this skill does not do

It does not call the SISTRIX API (as of v0.5.3, still true). CSV-first only.
...
## Appendix A — Conversion-rate validation layer

Referenced from §12 in the core analysis. If conversion rate (CR) or revenue data is available alongside the ranking data, the Monday check should include a CR validation that answers: is the recovery bringing the right traffic?
...
## Appendix B — Per-cluster stage classification

Referenced from §6 in the core analysis. Recovery does not progress uniformly across all keyword clusters. A site can be Stage 5 on its brand/authority cluster while Stage 1 on a competitive commercial cluster. The Monday check should output a stage classification **per keyword cluster**, not just a single global stage.

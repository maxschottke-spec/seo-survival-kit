...
- **DECISION_ENGINE fork resolved via the canonical-copy pattern** — the root `DECISION_ENGINE.md` and `plugins/seo-rescue/references/DECISION_ENGINE.md` had silently diverged (the plugin copy carries four newer decision rules — `r-batch-change-velocity-cap`, `r-shared-cms-layout-content-block`, `r-blog-authority-carries-recovery`, `r-high-impressions-low-ctr-page2` — that the root copy lacked). Same treatment as RECOVERY_SYSTEM/SISTRIX: the plugin copy is canonical (it is what skills and commands read), the root file is now a pointer stub with a 5-line abstract, and README/CLAUDE.md/ARCHITECTURE/ROADMAP links point at the canonical copy. The stale "v0.4.1 skills (10)" claim in the §11 status note corrected to the current 18. Interim step for #28 — whether the doc gets fully consolidated into RECOVERY_SYSTEM/ARCHITECTURE remains a v0.6 decision.

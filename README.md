...
**v0.5.4 shipped.** Public beta. Breaking changes possible before v1.0. Pin to a tag for reproducible installs. See [CHANGELOG.md](./CHANGELOG.md).
...
| [DECISION_ENGINE.md](./plugins/seo-rescue/references/DECISION_ENGINE.md) | Decision rules catalog, evidence weighting, data quality, profitability signals, prioritization, sequencing, cross-channel signals. Canonical copy lives in the plugin (`plugins/seo-rescue/references/`); the root [DECISION_ENGINE.md](./DECISION_ENGINE.md) is a pointer stub |
| [SISTRIX_MONDAY_RECOVERY_CHECK.md](./plugins/seo-rescue/references/SISTRIX_MONDAY_RECOVERY_CHECK.md) | Weekly CSV-first workflow specification (canonical copy; the root file is a pointer stub) — runnable skill at [`plugins/seo-rescue/skills/sistrix-monday-recovery-check/`](./plugins/seo-rescue/skills/sistrix-monday-recovery-check/) (v0.5.4) with synthetic example at [`examples/synthetic-sistrix-monday-check/`](./examples/synthetic-sistrix-monday-check/) |
